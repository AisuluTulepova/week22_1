...
window.addEventListener('load',async function(){
    try{
        let response = await this.fetch(`https://api.openweathermap.org/data/2.5/weather?q=Almaty&appid=e3ad77404dc88bee59f4a50942c3dbc6&units=metric`)
        if(!response.ok){
...
btn.addEventListener('click',(e)=>{
    ...
    console.log(city);
    if(city){
        sessionStorage.setItem('',JSON.stringify(city))
    }else{
